...
if status != "VALID":
    ...
    # FIXED: Clean HTML for description
    st.markdown("""
    <div style="background-color: #161B26; padding: 30px; border-radius: 15px; border: 1px solid #2B313E;">
        <h2 style="color: #4A90E2; margin-top: 0; text-align: center; margin-bottom: 25px;">🏆 Why Top Rankers Choose JEEx <span style="color:#4A90E2">PRO</span></h2>
        
        <div style="margin-bottom: 20px;">
            <strong style="color: #FFD700; font-size: 18px;">🧠 Advanced Problem Solving</strong><br>
            <span style="color: #CCCCCC;">Solves Irodov, Cengage, and PYQ level problems with step-by-step logic.</span>
        </div>

        <div style="margin-bottom: 20px;">
            <strong style="color: #FFD700; font-size: 18px;">👁️ Vision Intelligence (OCR)</strong><br>
            <span style="color: #CCCCCC;">Stuck on a handwritten question? Just upload a photo. JEEx reads and solves it.</span>
        </div>

        <div style="margin-bottom: 20px;">
            <strong style="color: #FFD700; font-size: 18px;">📄 Full Document Analysis</strong><br>
            <span style="color: #CCCCCC;">Upload PDF assignments. Our Code Interpreter analyzes the full context.</span>
        </div>

        <div>
            <strong style="color: #FFD700; font-size: 18px;">➗ Perfect Math Formatting</strong><br>
            <span style="color: #CCCCCC;">Renders complex integrals and matrices with LaTeX precision.</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.sidebar:
    ...
    if "messages" in st.session_state and len(st.session_state.messages) > 1:
        ...
        st.download_button("📥 Download PDF Notes", data=pdf_bytes, file_name="JEEx_Session.pdf", mime="application/pdf")
...
# --- 10. INPUT TOOLBAR (Gemini Style) ---
# We use a container to group the input elements
input_container = st.container()

with input_container:
    # Top Row: Tools (Voice + Attach)
    col_tools_1, col_tools_2 = st.columns([1, 15]) # Small column for voice, rest for attach spacing
    
    with col_tools_1:
        # VOICE INPUT (Native)
        # Using dynamic key to reset it after use
...
if audio_value:
    with st.spinner("Listening..."):
        try:
            # FORCE ENGLISH to stop Korean/Gibberish hallucinations
...
if prompt:
    ...
    with st.chat_message("assistant", avatar=LOGO_URL):
        ...
        st.session_state.audio_key += 1 # Reset audio widget so it doesn't repeat
        
        # Small delay to ensure UI update then rerun to clear input
        if audio_value: 
            time.sleep(0.5) 
            st.rerun()
